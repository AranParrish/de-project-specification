def format_data(result: list, db: str) -> dict | list:
    ...
    Args:   result  - list representing the output from a pg8000 query of a database
            db      - str representing the database connection used to return the query result
                    (required to get column headings)
    Return:     dict (for single row query result)
                or list of dict (for multiple row query results)
    """
